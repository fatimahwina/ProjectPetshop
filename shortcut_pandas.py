...
def generate_layanan_pdf(email_user):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", size=12)
    ...
    # Table Header
    pdf.cell(40, 10, txt="Waktu Order", border=1)
    pdf.cell(80, 10, txt="Jenis Layanan", border=1)
    pdf.cell(30, 10, txt="Tanggal Grooming", border=1)
    pdf.cell(40, 10, txt="Waktu Grooming", border=1)
    pdf.ln()
    ...
    with open(r'data\data_riwayat_grooming.csv', 'r') as file:
        reader = csv.reader(file)
        for row in reader:
            current_time, jenis_layanan, date, waktu_awal, email_address = row
            if email_address == email_user:
                transactions.append(row)
...
def send_layanan_with_pdf(email_user, pdf_filename):
    ...
    msg['To'] = email_user
    msg['Subject'] = "Laporan Transaksi Grooming KiwKiw Petshop"
...
def send_email_with_pdf(email_user, pdf_filename):
    ...
    msg['From'] = smtp_username
    msg['To'] = email_user
    msg['Subject'] = "Laporan Transaksi Pembelian Produk KiwKiw Petshop"
